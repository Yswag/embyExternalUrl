...
let serverAddr = 'http://example.com/';        //外網emby地址
const tags = ['BluRay', 'REMUX', 'WEB-DL'];                 //新增視訊tag
const groups = ['CMCT', 'WIKI', 'Z0N3', 'EbP', 'PTer', 'EPSILON', 'FRDS', 'SMURF'];   //新增製作組
...
const addExternalUrl = async (r, data, flags) => {
    ...
    //外鏈地址默認http,如果是https,則需要將這裡的 http 改為 https,如果是反代伺服器兩種都有,可以將這一行註釋掉,統一使用第一行填寫的地址
...
const getVlcUrl = (mediaInfo) => {
    ...
    //PC:
    //PC端需要額外設定,參考這個項目,MPV也是類似的方法:  https://github.com/stefansundin/vlc-protocol
